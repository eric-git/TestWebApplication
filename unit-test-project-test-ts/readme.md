...
the :environment for testing is defined in the section `rest-client.environmentVariables` in the [VS Code settings.json file](./.vscode/settings.json) and/or [http-client.env.json](./src/.http/http-client.env.json).

4. Open the relevant .http files under folder `.http` to test the requests.
...
- The Visual Studio environment configuration file [http-client.env.json](./src/.http/http-client.env.json) must be in sync with the section `rest-client.environmentVariables` in the [VS Code settings.json file](./.vscode/settings.json).
...
The information provided is as of 02/01/2025. Please update this document if any changes.
