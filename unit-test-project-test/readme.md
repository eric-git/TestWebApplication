...
1. Run the following command to get the client assertion that can be used for access token request without running the server;

```console
node ./src/tools/get-client-assertion.js -e <the environment name>
```

2. Run the following command to get the access token directly without running the server;

```console
node ./src/tools/get-access-token.js -e <the environment name>
```

3. Run the following command to get the decoded access token or client assertion without running the server;

```console
node ./src/tools/get-decoded-token.js -d <the access token or client assertion>
```

4. If installing `Postman Desktop` is not possible, e.g. running this project on a Linux system, then please install `VS Code Postman Extension` and import [the collection template](./src/assets/templates/Testing%20Node.js%20APIs.postman_collection.json) for testing the Node.js Web APIs;

5. Use the [.http file template](./src/assets/templates/template.http) to create new `.http` files.
