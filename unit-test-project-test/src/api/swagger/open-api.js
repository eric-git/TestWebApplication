...
const getJsonSpecificationsByVersionDetails = ({
  version,
  title,
  description,
  controllers,
}) => {
  const versionString = toVersionString(version);
  const patterns = controllers.map((x) => {
    return `${rootPath}/controllers/${x}/*(controller|route${versionString ? `.${versionString}` : ""}).js`;
  });
  const specification = swaggerJsdoc({
    definition: {
      ...
      servers: [
        {
          url: `/api${versionString ? `/${versionString}` : ""}`,
  ...
  if (version) {
    ...
    if (deprecatedVersions.some((x) => x === version)) {
      ensureHeaders(specification, {
    ...
    } else if (sunsetVersions.some((x) => x === version)) {
      ensureHeaders(specification, {
...
module.exports = {
  getJsonSpecificationsByVersionDetails,
};
